...
7. When the viewer must see a character speak an exact screenplay line, add the model-neutral `lip_sync` record with exact `LINE-###`, speaker, visibility, cut policy, and measured timing when available. Do not force lip sync onto off-screen dialogue.
8. Add `end_frame` only when chained generation, match on action, last-frame conditioning, or editorial continuity needs an explicit final state.
