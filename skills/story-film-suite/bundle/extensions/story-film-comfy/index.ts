...
const COMFY_DISCOVERY_BLOCK_REASON =
  "Story-Film blocks shell/filesystem ComfyUI discovery. Use story_comfy action=server-info for the live server, action=model-inventory or action=model-search for installed models, action=workflow-catalog for Story-Film and user workflows, and action=node-search or action=node-info for nodes. Story-Film does not search ComfyUI core/custom template catalogs. If the user wants one of those templates, they must save or copy it into their own ComfyUI workflow area first.";
