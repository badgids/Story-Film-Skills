...
If GitHub SSH authentication is already configured on this machine, this optional form is also valid:
...
```bash
pi install -l https://github.com/badgids/Story-Film-Skills.git@TAG_OR_COMMIT
```
...
```bash
cd /path/to/MyFilmProject
pi remove -l https://github.com/badgids/Story-Film-Skills.git
```
