...
const COMFY_DISCOVERY_BLOCK_REASON =
  "Story-Film blocks shell/filesystem ComfyUI discovery. Use story_comfy action=server-info for the live server, action=model-inventory or action=model-search for installed models, action=workflow-catalog for workflows/templates, and action=node-search or action=node-info for nodes. If story_comfy fails, report that managed-runtime failure or use only the documented Story-Film deterministic fallback. Do not scan guessed ComfyUI paths, home directories, model folders, or comfy-cli config files.";
